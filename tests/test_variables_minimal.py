...
def test_init_n_sims_upgrade_from_one():
    ...
    x = ProteusVariable("dim", {"a": 5, "b": StochasticScalar([1, 2, 3])})
    assert x.n_sims == 3
...
def test_reverse_lt():
    """Test reverse less than."""
    x = ProteusVariable("dim", {"a": 10, "b": 20})
    assert isinstance(15 < x, ProteusVariable)
...
def test_validate_freqsev_with_non_freqsev_value():
    ...
    x = ProteusVariable("dim", {"a": freq_sev, "b": 5})
...
def test_validate_freqsev_mismatched_sim_index():
    ...
    freq_sev2 = FreqSevSims(np.array([0, 0, 1]), np.array([10, 20, 30]), n_sims=3)
...
def test_setitem_string_key():
    """Test setting item with string key."""
