...
from pal.copulas import FRA1Copula, GaussianCopula, GumbelCopula
...
| FRA1 | `FRA1Copula` | `eta`, `theta`, `dimension` | Lower and upper |
| **Extreme value** | | | |
...
FRA1 also extends to any finite dimension. It is exchangeable: every pair has
the same FRA1 dependence structure, while the joint dependence is generated by
the common Archimedean frailty.

<!--pytest-codeblocks:cont-->

```python
set_random_seed(42)
fra1_samples = FRA1Copula(eta=0.3, theta=0.4, dimension=4).generate()
len(fra1_samples)  # => 4
```

For dimensions above two, PAL samples the positive frailty whose Laplace
transform is the FRA1 generator using numerical Laplace inversion, then applies
the Marshall-Olkin Archimedean sampling construction. The two-dimensional
implementation continues to use Hua's inverse conditional sampler.
