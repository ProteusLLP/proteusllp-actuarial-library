...
from pal.risk_measures import percentile_layer, tvar, var
...
total_capital_var = var(total_losses, 99.0).value
print(f"Total capital required at 99% VaR: {total_capital_var}")
allocated_capital = percentile_layer(total_losses, total_capital_var).allocate(losses)
...
allocated_capital_tvar = tvar(total_losses, 90).allocate(losses)
