...
print("Same group?", motor_losses.coupled_variable_group is property_losses.coupled_variable_group)
...
scatter_kwargs = {"mode": "markers", "marker": {"size": 2, "opacity": 0.3}}
pairs = [
...
for x_var, y_var, name, row, col in pairs:
    fig.add_trace(
        go.Scattergl(x=x_var.ranks, y=y_var.ranks, name=name, **scatter_kwargs),
        row=row,
        col=col,
    )
...
if not SUPPRESS_PLOTS:
    rank_scatter_fig.show()
    value_scatter_fig.show()
